...
struct SourceSelectionView: View {
    ...
    var body: some View {
        let sources = store.state.sourcesAndCalendars.map{$0.key}
        let calendars = store.state.sourcesAndCalendars.map{$0.value}
        List {
            ForEach(sources.indices) { idx in
                NavigationLink(destination: CalendarSelectionView(calendars: calendars[idx])
